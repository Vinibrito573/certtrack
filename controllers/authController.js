...
const login = async (req, res) => {
    ...
    res.cookie('token', token, {
      httpOnly: true, // for security reasons token is not accessible via JS
    ...
    return res.render('auth/login', { error: 'Something went wrong. Please try again.' }); //message
  }
};

// Handling logout
const logout = (req, res) => {
  res.clearCookie('token'); // removing the JWT cookie
  res.redirect('/login');
};
